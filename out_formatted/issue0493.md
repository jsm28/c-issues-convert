...
Status: Closed  
Cross-references: [0414](issue0414.md), [0469](issue0469.md), [0479](issue0479.md)  
...
5. The function description specifies that the `type` argument must have one of six values but lists only four. What are the remaining two values of the `type` argument? (Note: this problem is the subject of DR [479](issue0479.md).)
...
> resolution to [DR 469](issue0469.md) must include a Proposed Technical Corrigendum
> consistent with the answers provided below.
>
> As one example required to be resolved in [DR 469](issue0469.md),
>
...
> with those raised in [DR 469](issue0469.md) and [DR 479](issue0479.md).
...
> 5. See [DR 414](issue0414.md) for the resolution to the miscounted variations of `mtx_init()` options.
...
> 3. Calling `mtx_destroy()` while it is locked is intended to be undefined and will be resolved by [DR 469](issue0469.md).
